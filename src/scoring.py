...
from .trading.models import AnalysisContext, ScoredContext


def score_candidate(ctx: AnalysisContext, weights: Dict[str, float]) -> ScoredContext:
    """Explainable additive score. ctx contains tags + distances + momentum + trend metrics."""
    s = 0.0
    contrib: List[Dict[str, Any]] = []

    def add(key: str, cond: bool = True, val: Any = None) -> None:
        nonlocal s
        if not cond:
            return
    ...
    add("structure_bull", ctx.structure_bias == "BULL")
    add("structure_bear", ctx.structure_bias == "BEAR")
    add("bos_recent", (ctx.bos or {}).get("direction") is not None)

    d_ob = ctx.dist_to_ob_atr
    d_fvg = ctx.dist_to_fvg_atr
    add("ob_near", d_ob is not None and d_ob <= 1.0, d_ob)
    add("fvg_near", d_fvg is not None and d_fvg <= 1.0, d_fvg)
    add("ob_fvg_confluence", ctx.tag_confluence_ob_fvg)

    add("above_ma200", ctx.above_ma200)
    add("above_ma20", ctx.above_ma20)
    add("ma20_above_ma200", ctx.ma20_above_ma200)
    rsi = ctx.rsi14
    ...
    vol_adj_return_20 = ctx.vol_adj_return_20
    add("vol_adj_return_20", vol_adj_return_20 is not None and vol_adj_return_20 >= 1.0, vol_adj_return_20)

    return ScoredContext(**ctx.model_dump(), score=float(s), score_components=contrib)
